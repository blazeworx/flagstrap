    ...
    $.flagStrap = function (element, options) {
        ...
        var buildHtmlSelect = function () {
            ...
            if ($.isArray(plugin.countries)) {
                ...
                for(var i=0; i<plugin.countries.length; i++) {
                    ...
                    cObj = plugin.countries[i];

                    // get object keys (assume only one key)
                    cObjKeys = Object.keys(cObj);

                    // key and value
                    key = cObjKeys[0];
                    value = cObj[cObjKeys[0]];

                    optionAttributes = {value: key};
                    if (plugin.settings.selectedCountry !== undefined) {
                        if (plugin.settings.selectedCountry === key) {
                            optionAttributes = {value: key, selected: "selected"};
                            plugin.selected = {value: key, text: value};
                        }
                    }
                    htmlSelectElement.append($("<option>", optionAttributes).text(value));
                }
            } else {
        ...
        var buildDropDownButton = function () {
            ...
            if (selectedValue !== plugin.settings.placeholder.value) {
                ...
                $selectedLabel = $("<i/>").addClass("flagstrap-icon flagstrap-placeholder");
            }
